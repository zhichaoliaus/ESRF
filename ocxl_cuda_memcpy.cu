...
#include <sys/ipc.h>
#include <sys/shm.h>
#include "/home/ubuntu/zhichao/project/libocxl/src/include/libocxl.h"
...
{
	weq->queue = (memcpy_work_element *)aligned_alloc(getpagesize(), queue_size);;
...
void CudaPinMemoryTest(float* SrcOrigA, float* SrcOrigB, float* src, int N, size_t CudaSize)
{
	// Copy data to GPU
	printf(" - Copy data from Host to GPU ..\n");
	float* d_A;
	cudaMalloc(&d_A, CudaSize);
	float* d_B;
	cudaMalloc(&d_B, CudaSize);
	float* d_C;
	cudaMalloc(&d_C, CudaSize);
	cudaMemcpy(d_A, SrcOrigA, CudaSize, cudaMemcpyHostToDevice);
	cudaMemcpy(d_B, SrcOrigB, CudaSize, cudaMemcpyHostToDevice);
	printf(" - Running Kernel (array sum) in GPU ..\n");
	int threadsPerBlock = 256;
	int blocksPerGrid = (N + threadsPerBlock - 1)/ threadsPerBlock;
	VecAdd<<<blocksPerGrid, threadsPerBlock>>>(d_A, d_B, d_C, N);
	printf(" - Copy data from GPU to Host ..\n");
	cudaMemcpy(src, d_C, CudaSize, cudaMemcpyDeviceToHost);
	printf(" - First Three Returned Data: %f, %f, %f\n", src[0], src[1], src[2]);
	// Free device memory
	cudaFree(d_A);
	cudaFree(d_B);
	cudaFree(d_C);
...
void CudaZeroCopyTest(float* SrcOrigA, float* SrcOrigB, float* src, int N, size_t CudaSize)
{
	// Copy data to GPU
	printf(" - Copy data from Host to GPU ..\n");
	float* d_A;
	cudaHostAlloc((void**)&d_A, CudaSize, cudaHostAllocMapped);
	float* d_B;
	cudaHostAlloc((void**)&d_B, CudaSize, cudaHostAllocMapped);
	float* d_C;
	cudaHostAlloc((void**)&d_C, CudaSize, cudaHostAllocMapped);
	cudaMemcpy(d_A, SrcOrigA, CudaSize, cudaMemcpyHostToDevice);
	cudaMemcpy(d_B, SrcOrigB, CudaSize, cudaMemcpyHostToDevice);
	printf(" - Running Kernel (array sum) in GPU ..\n");
	int threadsPerBlock = 256;
...
void CudaUnifiedMemoryTest(float* SrcOrigA, float* SrcOrigB, float* src, int N, size_t CudaSize)
{
	// Copy data to GPU
	printf(" - Copy data from Host to GPU ..\n");
	float* d_A;
	cudaMallocManaged((void**)&d_A, CudaSize);
	float* d_B;
	cudaMallocManaged((void**)&d_B, CudaSize);
	float* d_C;
	cudaMallocManaged((void**)&d_C, CudaSize);
	cudaMemcpy(d_A, SrcOrigA, CudaSize, cudaMemcpyHostToDevice);
	cudaMemcpy(d_B, SrcOrigB, CudaSize, cudaMemcpyHostToDevice);
	printf(" - Running Kernel (array sum) in GPU ..\n");
	int threadsPerBlock = 256;
...
int shm_create(struct memcpy_test_args *args)
{
	...
	args->lock = (char*)shmat(args->shmid, NULL, 0);
	if (args->lock == (char *)-1) {
...
int test_afu_memcpy(struct memcpy_test_args *args)
{
	...
	float *src, *dst, *SrcOrigA, *SrcOrigB;
	int nevent;
	ocxl_err err;
	ocxl_afu_h afu_h;
	...
	ocxl_mmio_h pp_mmio;
	int N = 1024;
	size_t CudaSize = N * sizeof(float);
	...
	//SrcOrig = (char*)aligned_alloc(64, getpagesize());
	src = (float*)aligned_alloc(64, getpagesize());
	// Try define real data values
	SrcOrigA = (float*)malloc(CudaSize);
	SrcOrigB = (float*)malloc(CudaSize);
	// Initialize input vectors
  for (int i = 0; i < N; i++) {
    SrcOrigA[i] = (i+1);
		SrcOrigB[i] = 2*(i+1);
  }
	printf("Create %d-element Array in Host Memory:\n \tArray A First Three Data: %f, %f, %f\n\tArray B First Three Data: %f, %f, %f\n", N, SrcOrigA[0], SrcOrigA[1], SrcOrigA[2], SrcOrigB[0], SrcOrigB[1], SrcOrigB[2]);
	// Need to test mutiple GPU memory allocation options:
	// - Pinned memory (cudaMalloc)
	// - Zerocopy (cudaHostAlloc)
	// - Unified memory (cudaMallocManaged)
	printf("\n==> Start testing GPU pinned memory allocation...\n");
	gettimeofday(&start, NULL);
	CudaPinMemoryTest(SrcOrigA, SrcOrigB, src, N, CudaSize);
	gettimeofday(&end, NULL);
	t = (end.tv_sec - start.tv_sec)*1000000 + end.tv_usec - start.tv_usec;
  printf("Cuda Kernel Transfer & Calculation done in %d uS\n\n", t);

	printf("\n==> Start testing 2nd round GPU pinned memory allocation...\n");
	gettimeofday(&start, NULL);
	CudaPinMemoryTest(SrcOrigA, SrcOrigB, src, N, CudaSize);
	gettimeofday(&end, NULL);
	t = (end.tv_sec - start.tv_sec)*1000000 + end.tv_usec - start.tv_usec;
  printf("Cuda Kernel Transfer & Calculation done in %d uS\n\n", t);
	...
	printf("\n==> Start testing GPU ZeroCopy allocation...\n");
	gettimeofday(&start, NULL);
	CudaZeroCopyTest(SrcOrigA, SrcOrigB, src, N, CudaSize);
	gettimeofday(&end, NULL);
	t = (end.tv_sec - start.tv_sec)*1000000 + end.tv_usec - start.tv_usec;
  printf("Cuda Kernel Transfer & Calculation done in %d uS\n\n", t);

	printf("\n==> Start testing 2nd round GPU ZeroCopy allocation...\n");
	gettimeofday(&start, NULL);
	CudaZeroCopyTest(SrcOrigA, SrcOrigB, src, N, CudaSize);
	gettimeofday(&end, NULL);
	t = (end.tv_sec - start.tv_sec)*1000000 + end.tv_usec - start.tv_usec;
  printf("Cuda Kernel Transfer & Calculation done in %d uS\n\n", t);
	...
	printf("\n==> Start testing GPU/CPU unified memory allocation...\n");
	gettimeofday(&start, NULL);
	CudaUnifiedMemoryTest(SrcOrigA, SrcOrigB, src, N, CudaSize);
	gettimeofday(&end, NULL);
	t = (end.tv_sec - start.tv_sec)*1000000 + end.tv_usec - start.tv_usec;
  printf("Cuda Kernel Transfer & Calculation done in %d uS\n\n", t);

	printf("\n==> Start testing 2nd round GPU/CPU unified memory allocation...\n");
	gettimeofday(&start, NULL);
	CudaUnifiedMemoryTest(SrcOrigA, SrcOrigB, src, N, CudaSize);
	gettimeofday(&end, NULL);
	t = (end.tv_sec - start.tv_sec)*1000000 + end.tv_usec - start.tv_usec;
  printf("Cuda Kernel Transfer & Calculation done in %d uS\n\n", t);
	...
	if (args->atomic_cas) {
		...
		dst = (float*)mmap(NULL, getpagesize(), PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
		if (dst == MAP_FAILED) {
